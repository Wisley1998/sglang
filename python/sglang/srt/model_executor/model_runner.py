...
class ModelRunner:
    ...
    def _maybe_execute_gdn_lfc_reconstructions(self, forward_batch: ForwardBatch):
        """Pre-forward GDN LFC reconstruction: fuse all layers into one kernel launch.

        Instead of launching one batched kernel per GDN layer during the model forward
        pass (interleaved with model computation), this runs all GDN reconstructions
        upfront in a single kernel call.
        """
        # Quick exit: no reconstruction factors or not a GDN model
        lfc_factors = getattr(forward_batch, "lfc_reconstruction_factors", None)
        if not lfc_factors:
            return

        # Only applies to HybridLinearAttnBackend with GDN
        ...
        # Get the linear backend's cache_indices (set by init_forward_metadata)
        linear_backend = attn_backend.linear_attn_backend
        fwd_meta = getattr(linear_backend, "forward_metadata", None)
        if fwd_meta is None:
            return

        # Determine GDN layer IDs (all non-full-attention layers)
        full_attn_set = set(attn_backend.full_attn_layers)
        num_layers = self.model_config.num_hidden_layers
        gdn_layer_ids = [i for i in range(num_layers) if i not in full_attn_set]
        if not gdn_layer_ids:
            return

        # Get temporal tensor and mamba_map
        rtp = self.req_to_token_pool
        if not hasattr(rtp, "mamba_pool") or not hasattr(rtp, "mamba_map"):
            return
        temporal = rtp.mamba_pool.mamba_cache.temporal
        mamba_map = rtp.mamba_map

        # Get cache indices list (convert once, cache on forward_batch)
        num_reqs = fwd_meta.query_start_loc.shape[0] - 1
        if not hasattr(forward_batch, '_lfc_gdn_cache_indices_list'):
            forward_batch._lfc_gdn_cache_indices_list = (
                fwd_meta.mamba_cache_indices[:num_reqs].tolist()
            )

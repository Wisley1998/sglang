...
class Envs:
    ...
    # LFC (Linear Factor Caching) catch-up timing instrumentation
    SGLANG_CATCHUP_TIMING = EnvBool(False)  # Enable timing collection
    SGLANG_CATCHUP_TIMING_DIR = EnvStr("/tmp/sglang_catchup")  # Output directory
    SGLANG_LFC_ENABLED = EnvBool(False)  # Enable LFC mode
    SGLANG_SNAPSHOT_INTERVAL = EnvInt(500)  # K value for snapshot interval
    SGLANG_LFC_MEMORY_BUDGET_GB = EnvFloat(2.0)  # GPU memory budget for LFC factors
